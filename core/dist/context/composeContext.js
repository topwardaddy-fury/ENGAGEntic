...
const normalizeContext_1 = require("./normalizeContext");
async function composeContext(params, loaders) {
    ...
    async function resolveArtifact(idOrPath, visited = new Set()) {
    ...
    async function processIncludes(artifact, visited) {
        if (artifact.includes && Array.isArray(artifact.includes)) {
            for (const inclId of artifact.includes) {
                ...
                if (incl) {
                    if (incl.type === 'standard' && !layers.standards.some((s) => s.name === `Standard: ${incl.name || incl.id}`)) {
    ...
    if (profile_id) {
        let currentProfileId = profile_id;
        const profileStack = [];
    ...
    if (selectedStandardIds && Array.isArray(selectedStandardIds)) {
        for (const id of selectedStandardIds) {
            ...
            if (std) {
                if (!layers.standards.some((s) => s.name === `Standard: ${std.name || std.id}`)) {
    ...
    return (0, normalizeContext_1.normalizeContext)(layers, {
