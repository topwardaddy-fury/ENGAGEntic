...
async function githubReadmeText(owner: string, name: string): Promise<string> {
    ...
    for (const query of queryQueue) {
        ...
        for (let page = 1; page <= 4; page += 1) {
            ...
            for (const repo of items) {
                ...
                if (!fullName || excludeRepos.has(fullName) || seenRepos.has(fullName)) continue;
...
function extractRepoLeaf(fullName: string): string {
    const parts = fullName.split('/');
    return parts[parts.length - 1] || fullName;
}

function extractReadmeBullets(readme: string, maxItems = 8): string[] {
    ...
    for (const line of lines) {
        ...
        if (text.length < 18 || text.length > 180) continue;
        bullets.push(text);
    }
    return bullets;
}

function extractReadmeHeadings(readme: string, maxItems = 5): string[] {
...
function decomposeCandidate(candidate: ScoutCandidate): {
    profileName: string;
    ...
    const profileDescription = compactText(
        candidate.description || `Reusable role baseline derived from ${candidate.fullName}.`,
        180
    );
    const standardDescription = compactText(
        `Actionable standards extracted from ${candidate.fullName} for repeatable, secure AI work.`,
        180
    );
    const specDescription = compactText(
        `Operational specification that composes the Community profile and standards for ${candidate.fullName}.`,
        180
    );
